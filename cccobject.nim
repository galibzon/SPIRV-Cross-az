...
{.emit:["""/*TYPESECTION*/
template<typename T>
void callCCCPtrDestructor(T* instance) { instance->~T(); }
  """].}
...
when isMainModule:
  ...
  type
    MyClass {.header:"MyClass.hpp", importcpp:"MyClass".} = object
    MyClass2 {.header:"MyClass.hpp", importcpp:"MyClass2".} = object

  proc isCCCConcept(T: typedesc[MyClass]): bool = true
  proc isCCCConcept(T: typedesc[MyClass2]): bool = true
    
  proc test(self: MyClass, x: cint): cint {.importcpp:"#.test(#)".}
  # expandMacros:
  #   dynamic:
  # dumpAstGen:
  when true:
